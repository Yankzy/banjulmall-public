    ...
    <div className="flex flex-col justify-center items-center h-full w-full bg-white px-5">
      ...
      <div className="relative flex flex-row justify-between items-center w-full h-20 bg-[ededed] shadow-sm overflow-visible">
          ...
          <p className='text-2xl cursor-pointer z-20'>
            Checkout (
            <span onClick={()=> router.push('/cart')} className=' text-bm_teal-light cursor-pointer'>
              {items.length} items
            </span>)
          </p>
      ...
      <div className='relative flex flex-row w-full items-start justify-between px-52 py-5'>
        ...
        <div className='flex flex-col w-[30%] border rounded-md border-gray-300 top-0 shadow-md'>
          {paymentMethod ? (
            <>
              <button className='bg-yellow-300 border border-yellow-100 rounded-md p-2 text-xs mx-3 mt-2'>Place your order</button>
              <p 
            ...
            <>
              <button className='bg-yellow-300 border border-yellow-100 rounded-md p-2 text-xs mx-3 mt-2'>Use this payment method</button>
              <p className=' text-xs items-center justify-center py-3 mx-3 leading-normal'>Choose a payment method to continue checking out. You'll still have a chance to review and edit your order before it's final.</p>
            </>
          )}
          <hr />
          <div className='flex flex-col m-3'>
            ...
            <div className='flex flex-row justify-between items-start w-full mb-4'>
              <div className='flex flex-col justify-start items-start w-full'>
                <p className='text-xs mb-2'>Items ({items.length})</p>
                <p className='text-xs mb-2'>Transport</p>
                <p className='text-xs mb-2'>VAT</p>
              </div>
              <div className='flex flex-col justify-end items-end w-full text-right'>
                <p className='text-xs mb-2'>${items.reduce((acc, item) => acc + item.price * item.quantity, 0).toFixed(2)}</p>
                <p className='text-xs mb-2'>Free</p>
                <p className='text-xs mb-2'>${(items.reduce((acc, item) => acc + item.price * item.quantity, 0) * 0.15).toFixed(2)}</p>
              </div>
            </div>
            <hr />
            <div className='flex flex-row justify-between items-center w-full mt-2 text-lg font-bold text-red-800'>
              ...
              <p className='text-right'>
                ${(
                  items.reduce((acc, item) => acc + item.price * item.quantity, 0) * 1.15
                ).toFixed(2)}
              </p>
            </div>
          </div>
